...
public class EndColumnsFeature extends Feature<BasaltColumnsFeatureConfig> {
    ...
    public EndColumnsFeature(Codec<BasaltColumnsFeatureConfig> codec) {
        super(codec);
    }
        ...
        while(true) {
            ...
            for(BlockPos.Mutable mutable = blockPos2.mutableCopy(); j >= 0; --j) {
                if (isAirOrLavaOcean(world, seaLevel, mutable)) {
                    this.setBlockState(world, mutable, ModBlocks.DARK_PURPUR.getDefaultState());
                    mutable.move(Direction.UP);
                    bl = true;
                } else {
                    if (!world.getBlockState(mutable).isOf(ModBlocks.DARK_PURPUR)) {
                        break;
                    }
